...
// Firmware version
String FW_Version = "科睿 - 小蜘蛛 v2.0";

#define RGBPIN  0     // led dat pin
#define EYES    2     // number of used RGB diodes
Adafruit_NeoPixel Roboteyes = Adafruit_NeoPixel(EYES, RGBPIN, NEO_GRB + NEO_KHZ800);
...
{
  Serial.begin(9600);
  Serial.println("Rboot Start!");

  Roboteyes.begin(); // library initialization
  for(int i=0;i<EYES;i++){
    Roboteyes.setPixelColor(i,0, 0, 100); 
    Roboteyes.show();
  }
  ...
  String AP_NameString = "Spider Robot";
...
void Breath(void)
{
  int arry[3]={0,0,0};
  int tempint=0;
  if(millis()<(TimeOut+90)) return;
  TimeOut=millis();
  switch(BreathStep)
  {
    case 0:
    {    
      Rpi=ColorArry[ColorArryPoint][0]/30;
      Gpi=ColorArry[ColorArryPoint][1]/30;
      Bpi=ColorArry[ColorArryPoint][2]/30;
      BreathStep=1;
    }break;
    case 1:
    {
      tempint=LedCount*Rpi;
      if(tempint<ColorArry[ColorArryPoint][0])
      {
        arry[0]=tempint;
      }
      else
      {
        arry[0]=ColorArry[ColorArryPoint][0];
      }
      tempint=LedCount*Gpi;
      if(tempint<ColorArry[ColorArryPoint][1])
      {
        arry[1]=tempint;
      }
      else
      {
        arry[1]=ColorArry[ColorArryPoint][1];
      }
      tempint=LedCount*Bpi;
      if(tempint<ColorArry[ColorArryPoint][2])
      {
        arry[2]=tempint;
      }
      else
      {
        arry[2]=ColorArry[ColorArryPoint][2];
      }
      
      for(int i=0;i<EYES;i++)
      {
        Roboteyes.setPixelColor(i,arry[0], arry[1], arry[2]); 
        
      }Roboteyes.show();
      if(LedCount<30) LedCount++;
      else 
      {
        LedCount=0;
        BreathStep=2;
    ...
    {      
      tempint=LedCount*Rpi;
      if(ColorArry[ColorArryPoint][0]>tempint)
      {
        arry[0]=ColorArry[ColorArryPoint][0]-tempint;
      }
      else
      {
        arry[0]=0;
      ...
      tempint=LedCount*Gpi;
      if(ColorArry[ColorArryPoint][1]>tempint)
      {
        arry[1]=ColorArry[ColorArryPoint][1]-tempint;
      }
      else
      {
        arry[1]=0;
      ...
      tempint=LedCount*Bpi;
      if(ColorArry[ColorArryPoint][2]>tempint)
      {
        arry[2]=ColorArry[ColorArryPoint][2]-tempint;
      }
      else
      {
        arry[2]=0;
      ...
      else 
      {
        ...
        BreathStep=0;
        ColorArryPoint++;
        if(ColorArryPoint==7) ColorArryPoint=0;
        arry[0]=0;
        arry[1]=0;
        arry[2]=0;
      }
...
void loop(void)
{
  ...
  if (Servo_PROGRAM >= 1 ) {
    switch (Servo_PROGRAM) {
      case 1: // Standby 待機
        for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,0, 0, 0); 
          Roboteyes.show();
        }
      ...
      case 2: // Forward 前行
        for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,0, 255, 0); 
          Roboteyes.show();
        }
      ...
      case 4: // Left shift 左移
        for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,200, 200, 0); 
          Roboteyes.show();
        }
      ...
      case 6: // Turn left 左轉
         for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,200, 0, 200); 
          Roboteyes.show();
        }
      ...
      case 8: // Lie 趴地
         for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,0, 100, 0); 
          Roboteyes.show();
        }
      ...
      case 9: // Say Hi 打招呼
         for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,100, 100, 0); 
          Roboteyes.show();
        }
      ...
      case 10: // Fighting 戰鬥姿態
        for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,255, 0, 0); 
          Roboteyes.show();
        }
      ...
      case 11: // Push up 掌上壓
        for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,100, 200, 0); 
          Roboteyes.show();
        }
      ...
      case 12: // Sleep 睡眠姿勢
        for(int i=0;i<EYES;i++){
          Roboteyes.setPixelColor(i,100, 0, 100); 
          Roboteyes.show();
        }
        Servo_PROGRAM_Run(Servo_Prg_1, Servo_Prg_1_Step);
